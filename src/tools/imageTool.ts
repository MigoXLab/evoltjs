...
import { UserMessage } from '../schemas/message';
...
export class ImageTool {
     ...
     */
    async readImage(imagePath: string, instruction: string = ''): Promise<UserMessage> {
        try {
            ...
            if (!isSupportedImageFile(imagePath)) {
                ...
                return new UserMessage({
                    content: `Error: Unsupported image format '${imagePath}'. Only JPEG, PNG, GIF, and WebP are supported by the AI model.`,
                });
            }
            ...
            return new UserMessage({
                content: `Error reading image ${imagePath}: ${error}`,
            });
        }
    }
     ...
     */
    async readImages(imagePaths: string[], instruction: string = ''): Promise<UserMessage> {
        try {
            ...
            if (supportedPaths.length === 0) {
                return new UserMessage({
                    content: `No supported image formats found in the provided list. Only JPEG, PNG, GIF, and WebP are supported. Unsupported files: ${unsupportedPaths.join(', ')}`,
                });
            }
            ...
            if (instruction) {
                contentParts.push(`Instruction: ${instruction}`);
            }
            if (unsupportedPaths.length > 0) {
            ...
            return new UserMessage({
                content: `Error reading images ${imagePaths.join(', ')}: ${error}`,
            });
        }
    }
}
