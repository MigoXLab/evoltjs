...
/**
 * Helper function to validate and normalize AgentConfig
 */
export function normalizeAgentConfig(config: AgentConfig): AgentConfig {
    return {
        ...
        observeTimeout: config.observeTimeout ?? 60.0,
        workspaceDir: config.workspaceDir || '',
        autoShutdownExecutor: config.autoShutdownExecutor ?? false,
        skills: config.skills || [],
        rulePaths: config.rulePaths || [],
        toolcallManagerPoolSize: config.toolcallManagerPoolSize ?? 5,
    };
}
